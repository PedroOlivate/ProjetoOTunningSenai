...
const preencherTamanhos = (key) => {
    ...
    selecionaTodos('.pecaInfo--size').forEach((size, sizeIndex) => {
        ...
        size.querySelector('span').innerHTML = pecaJson11[key].sizes[sizeIndex]
    })
}
...
const escolherTamanhoPreco = (key) => {
    ...
    selecionaTodos('.pecaInfo--size').forEach((size, sizeIndex) => {
        size.addEventListener('click', (e) => {
            ...
            seleciona('.pecaInfo--actualPrice').innerHTML = formatoReal(pecaJson11[key].price[sizeIndex])
        })
    })
}
...
const adicionarNoCarrinho = () => {
    seleciona('.pecaInfo--addButton').addEventListener('click', () => {
	    ...
	    let identificador = pecaJson11[modalKey].id+'t'+size
...
const atualizarCarrinho = () => {
	...
	if(cart.length > 0) {
		...
		for(let i in cart) {
			...
			let pecaItem = pecaJson11.find( (item) => item.id == cart[i].id )
