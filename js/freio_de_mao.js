...
const preencherTamanhos = (key) => {
    ...
    selecionaTodos('.pecaInfo--size').forEach((size, sizeIndex) => {
        ...
        size.querySelector('span').innerHTML = pecaJson4[key].sizes[sizeIndex]
    })
}
...
const escolherTamanhoPreco = (key) => {
    ...
    selecionaTodos('.pecaInfo--size').forEach((size, sizeIndex) => {
        size.addEventListener('click', (e) => {
            ...
            seleciona('.pecaInfo--actualPrice').innerHTML = formatoReal(pecaJson4[key].price[sizeIndex])
        })
    })
}
...
const adicionarNoCarrinho = () => {
    seleciona('.pecaInfo--addButton').addEventListener('click', () => {
	    ...
	    let identificador = pecaJson4[modalKey].id+'t'+size
...
const atualizarCarrinho = () => {
	...
	if(cart.length > 0) {
		...
		for(let i in cart) {
			...
			let pecaItem = pecaJson4.find( (item) => item.id == cart[i].id )
