...
const preencherTamanhos = (key) => {
    ...
    selecionaTodos('.pecaInfo--size').forEach((size, sizeIndex) => {
        ...
        size.querySelector('span').innerHTML = pecaJson3[key].sizes[sizeIndex]
    })
}
...
const escolherTamanhoPreco = (key) => {
    ...
    selecionaTodos('.pecaInfo--size').forEach((size, sizeIndex) => {
        size.addEventListener('click', (e) => {
            ...
            seleciona('.pecaInfo--actualPrice').innerHTML = formatoReal(pecaJson3[key].price[sizeIndex])
        })
    })
}
...
const adicionarNoCarrinho = () => {
    seleciona('.pecaInfo--addButton').addEventListener('click', () => {
	    ...
	    let identificador = pecaJson3[modalKey].id+'t'+size
...
const atualizarCarrinho = () => {
	...
	if(cart.length > 0) {
		...
		for(let i in cart) {
			...
			let pecaItem = pecaJson3.find( (item) => item.id == cart[i].id )
